...
The AI mentor talks to any OpenAI-compatible `chat/completions` endpoint. Compact mode uses graph counts, selection metadata captured for the turn, a fixed policy that treats graph-derived content as reference data and keeps mentor capabilities read-only, tool routing and temporary tool context, and a short review-priority rule: lower stability and `Again` or `Hard` suggest weaker recall, while `isDue` is scheduling context. Visible text history is sent with the request. The full FSRS legend remains in the legacy fallback and tool-specific guidance; tool-capable models can request bounded, read-only queries against the live graph, and tool inputs and results stay transient (never rendered, logged, or persisted). Endpoints that reject tool calls are retried once with a legacy graph snapshot.
