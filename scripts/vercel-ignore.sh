...
docs)
  ...
  # pulls vocab-learning + schema transitively, so all three count;
  # lockfile/workspace changes can alter the install too.
  paths=(docs packages/graph packages/vocab-learning packages/schema pnpm-lock.yaml pnpm-workspace.yaml)
  ;;
app)
