...
The branch-heavy pure graph queries in `src/llm/tools.ts` have their own Vitest file floor (`93/90/86/92` for statements/branches/functions/lines). `src/llm/context.ts`, including the exported 4,000-character persona boundary and 12,000-character legacy prompt ceiling, and `src/data/fsrsDueQueue.ts` retain their existing file floors. Mutation testing keeps the areas separate: the legacy `mentor` area mutates prompt context, deterministic opaque graph handles, and due-queue logic at `break` 84, while `graphTools` mutates only `src/llm/tools.ts:33-242` at `break` 91. The SDK adapter tail of `tools.ts` and `src/llm/completion.ts` are transport/integration glue and are outside mutation testing.
