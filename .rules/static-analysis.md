...
Trust boundary (`grant_fs_scope` / `pick_workspace_folder`): Rust owns the picker dialog via the **callback-based** `pick_folder` bridged through `await_folder_picker` — the Tauri runtime stays unblocked while the native dialog is open (no `blocking_pick_folder`). The command persists approved paths in a trust store outside the renderer's fs scope and rejects non-app-data paths not under a previously-trusted root. App-data auto-grants are restricted to `.nesso` subtrees; the project root is trusted through the seeded trust store, not app-data prefix matching. The Rust tests cover `is_path_safe_for_grant`, `is_path_trusted`, `validate_picked_folder`, trust-store descendants, the trust-store file itself, traversal, arbitrary-external, home/root/hidden, picker-validation, rejection paths, and the existing canonicalization/symlink/CSP contracts remain reused. Five focused regression tests (`picker_approval_tests`) cover the approval helper (`try_approve_folder`) with injected callbacks at the helper level: runtime progress while pending, successful grant-before-persist ordering, cancellation with no side effects, invalid selection with no side effects, and scope-grant failure without persistence. `save_file_dialog` uses `blocking_save_file` on the Rust side so the renderer never provides an export path; it remains out of scope for the non-blocking rework. A native CSP test validates `tauri.conf.json` rejects unrestricted `http:` sources.
