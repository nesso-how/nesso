...
Trust boundary (`grant_fs_scope` / `pick_workspace_folder` / `save_file_dialog`): Rust owns both path dialogs via the **callback-based** `pick_folder` / `save_file` bridged through the shared `await_native_path_dialog` — the Tauri runtime stays unblocked while the native dialog is open. The renderer never provides an export path; Rust owns both the dialog and the write. After dialog completion the synchronous file write is dispatched through `tauri::async_runtime::spawn_blocking` — never inside the dialog callback. Blocking native path-dialog APIs (`blocking_pick_folder`, `blocking_save_file`, etc.) are forbidden due to Tauri/macOS event-loop risk. The `pick_workspace_folder` command persists approved paths in a trust store outside the renderer's fs scope and rejects non-app-data paths not under a previously-trusted root. App-data auto-grants are restricted to `.nesso` subtrees; the project root is trusted through the seeded trust store, not app-data prefix matching. The Rust tests cover `is_path_safe_for_grant`, `is_path_trusted`, `validate_picked_folder`, trust-store descendants, the trust-store file itself, traversal, arbitrary-external, home/root/hidden, picker-validation, rejection paths, and the existing canonicalization/symlink/CSP contracts remain reused. Five focused regression tests (`picker_approval_tests`) cover the approval helper (`try_approve_folder`) with injected callbacks at the helper level: successful grant-before-persist ordering, cancellation with no side effects, invalid selection with no side effects, scope-grant failure without persistence, and non-UTF-8 path rejection before side effects. Bridge yield, callback-error, and cancellation/drop tests are separate. The save-export helper (`write_selected_export`) is covered at the helper level by four focused Rust regression tests (write, cancel, error path, non-UTF-8 rejection). The dialog-bridge helper (`await_native_path_dialog`) is covered by a callback-error-wrapping test and a non-blocking-yield regression test. A native CSP test validates `tauri.conf.json` rejects unrestricted `http:` sources.
...
The branch-heavy pure graph queries in `src/llm/tools.ts` retain their Vitest file floor (`93/90/86/92` for statements/branches/functions/lines). The explicitly layered prompt composer in `src/llm/context.ts` covers persona, fixed policy, compact runtime, legacy runtime, the 4,000-character persona boundary, and the 12,000-character legacy ceiling under its existing file floor. Mutation testing remains split: `mentor` mutates prompt context, graph handles, and due-queue logic at `break` 84, while `graphTools` mutates only `src/llm/tools.ts:31-240` at `break` 91. Tool-description adapters and `src/llm/completion.ts` remain outside mutation testing.

The direct scripts are `pnpm run analyze:mutation:mentor` and `pnpm run analyze:mutation:graphTools`; the aggregate `pnpm run analyze:mutation` runs both alongside schema/store/workspace. `pnpm run analyze:mutation:changed` routes changed files through `scripts/stryker/areas.mjs`: tool source/tests select `graphTools`, while context, graph-handle, and due-queue paths select `mentor`. Mutation testing is an opt-in/preflight or scheduled non-blocking check, not a required per-PR CI job. The required JS CI gates remain format, security headers, lint, license headers, `pnpm test:coverage`, type coverage, package/app builds, and the three Fallow gates. `pnpm run fast-check` is the lighter task loop of format, lint, type-check, and tests.
