...
The mentor's network transport (`fetchCompletion`) lives in `src/llm/completion.ts` and is tested against a **fully stubbed transport layer**. No real HTTP calls ever leave Vitest. The transport has exactly two paths (`isDesktop()` → Tauri `@tauri-apps/plugin-http` versus browser global `fetch`); each transport test pins that the right one fires with the right shape.

The same transport suite covers optional AI SDK tools. Its OpenAI-compatible SSE fixture emits a `delta.tool_calls` chunk followed by `finish_reason: 'tool_calls'`; the test asserts that the SDK executes the tool, invokes `onToolCall`, makes the second request, and sends the assistant `tool_calls` message plus the `role: 'tool'` result message on that request. The four-step test emits four tool-call responses and asserts exactly four model requests: `isStepCount(4)` is a total model-step ceiling, not four tool calls plus a final answer. Tool messages are SDK-owned within one `fetchCompletion` invocation and are not manually added to the panel history.
...
`src/llm/context.test.ts` owns exact persona/fixed-policy/runtime composition. It pins the built-in concise Socratic/read-only wording and the absence of the removed graph-organization prohibition, lecturing comparison, and one-question/~180-word target; built-in and custom personas in compact and legacy modes; blank and whitespace fallback; the 4,000-character persona boundary; consolidated provenance; delimiter neutralization; compact omission of eager graph content and the full FSRS legend; legacy self-containment, snapshot bounds, and the complete 12,000-character ceiling. `src/components/mentor/MentorPanel.test.tsx` continues to own compatibility retry and captured-turn behavior.

`src/llm/tools.test.ts` pins the context-owned `FSRS_PRIORITY_RULE`, exact `getGraphOverview` and `inspectConcept` descriptions, weakest-first/FSRS interpretation, successful `contentProvenance`, unchanged result shapes, bounds, schemas, six-tool read-only surface, and live getter execution. It stays in the default Node environment.
...
`pnpm test:coverage` runs as a required JS step in `.github/workflows/ci.yml`, gating PRs alongside the checks described in [static-analysis](static-analysis.md). The JS lane also runs format, security-header, lint, license-header, type-coverage, package/app builds, and the three Fallow gates; the E2E lane runs Playwright when its path filter selects it, and the Rust lane runs its native gates when selected. Package `dist` is in place because the prior `pnpm install --frozen-lockfile` step runs `prepare`; the release web job skips only the duplicate root prepare build because `vercel build` owns the production build.
...
Glob granularity is deliberate. A folder gets a directory floor only when it is pure testable logic end to end. Where a folder mixes tested logic with code this layer cannot reach, the floor targets the **file**, not the folder: `packages/graph/src/geometry.ts` (edge-curve math — the rest of `packages/graph` is React rendering only the e2e layer #28 reaches), `src/lib/graphClipboard.ts` / `src/lib/graphId.ts` (the rest of `src/lib` is untested desktop/IO glue), and `src/llm/context.ts`, `src/llm/tools.ts`, and `src/data/fsrsDueQueue.ts` (the full `src/llm/tools.ts` file is held to its Vitest coverage floor; only its thin SDK adapter tail is excluded from Stryker mutation scope; `completion.ts`'s network transport and `src/data`'s seed/storage glue aren't held to the same bar). An aggregate floor over those folders would be diluted to ~31% / ~33% and guard nothing. Same reason the store glue `src/store/index.ts` sits outside the `src/store/slices/**` floor. Files covered only **incidentally** by other suites (`src/data/seedGraph.ts`, parts of `src/store/db.ts`) get no floor — their numbers move with unrelated tests. Packages with no meaningful unit surface stay global-only: `mcp` (loaders/tooling), `vocab-learning` (static vocabulary + thin serialize wrapper).

Mentor-specific file floors in `vitest.config.ts` are the current ratchet values, not aspirational targets:

| File                       | Statements | Branches | Functions | Lines |
| -------------------------- | ---------: | -------: | --------: | ----: |
| `src/llm/context.ts`       |         95 |       92 |        95 |    95 |
| `src/llm/tools.ts`         |         93 |       90 |        86 |    92 |
| `src/data/fsrsDueQueue.ts` |         95 |       90 |        95 |    95 |

The full `src/llm/tools.ts` Vitest floor remains in force; Stryker's `graphTools` area excludes only the thin SDK adapter tail. Raise floors only from a fresh measured run when the tested surface improves; if a deliberate surface change lowers one, re-baseline in the same change.
...
**WebDriver cannot drive native OS windows** — the folder/save **dialogs** (`pickWorkspaceFolder`, `save_file_dialog`) and the **native menu bar** are not DOM, so multi-project switching, native import/export, and menu commands stay covered by Vitest (`graph-management.desktop.test.ts`, `useDesktopMenu`), not this lane. Both dialogs are Rust-owned and callback-based, bridged through the shared `await_native_path_dialog` so the Tauri runtime stays unblocked; blocking native path-dialog APIs (`blocking_pick_folder`, `blocking_save_file`, etc.) are forbidden due to Tauri/macOS event-loop risk. Export writes are dispatched through `spawn_blocking` after dialog completion — never inside the dialog callback. The folder-picker approval helper (`try_approve_folder`) is covered at the helper level by five focused Rust regression tests (`picker_approval_tests`) with injected callbacks: successful grant-before-persist ordering, cancellation with no side effects, invalid selection with no side effects, scope-grant failure without persistence, and non-UTF-8 path rejection before side effects. The save-export helper (`write_selected_export`) is covered at the helper level by four Rust regression tests (write, cancel, error path, non-UTF-8 rejection). The dialog-bridge (`await_native_path_dialog`) is covered by a callback-error-wrapping test, a non-blocking-yield regression test, and a callback-cancellation-vs-drop test. The external-conflict **banner** branch (unsaved local edits + an external write) needs deterministic timing control and is a follow-up; the no-local-edits reload branch is covered.
...
- **Timing:** the autosave is debounced 500ms (`useAutoSave`) and the fs watcher 400ms (`useGraphFileWatch`); selection syncs via `requestAnimationFrame`. A flow that reloads/switches graphs or asserts on disk must wait for the flush — the native helpers poll the filesystem (`waitForCondition`). Chromium's File System Access API (`showSaveFilePicker`) blocks driving a real JSON export in the web lane — the export spec deletes it via `addInitScript` to force the anchor-download fallback. The PNG export regression intercepts only the Vite root-document response, applies the exact catch-all CSP read from `vercel.json`, captures in-page `securitypolicyviolation` events by `effectiveDirective` (`img-src` / `font-src`) — the reliable signal, since Chromium redacts blocked `data:` URLs to the literal `"data"` in `blockedURI` — tokenizes CSP directives independently of source order, and asserts both the PNG signature and the absence of those violations; the security-header unit suite separately pins every required source token.
- **Review reminder seeding:** to make a durable previously reviewed due concept, `makeCurrentGraphConceptStudiedAndDue` reads the current graph id from persisted Zustand state, finds the first node in IndexedDB `graphs`, and writes a reviewed record (`reps: 1`, a past `due`, and review metadata) directly to the `reviewState` object store before reloading. Reloading lets the normal FSRS merge and `loadedToken` path run; do not mutate graph JSON or rely on elapsed time to create the reviewed card.
...
- **Per-area configs, per-area floors.** A shared base (`scripts/stryker/base.mjs`) plus one config per area (`scripts/stryker/<area>.mjs`, built via `area()`), each with its own `mutate` and `break`. Thresholds are **per area, not aggregated**: a tiny well-tested package would otherwise mask a large less-tested one (the same reason `vitest.config.ts` keeps per-directory coverage floors). `mutate` is pure logic only — UI/React and the Rust layer stay out of scope. The legacy `mentor` area mutates `src/llm/context.ts` and `src/data/fsrsDueQueue.ts` at `break` 84. The separate `graphTools` area mutates only the pure-query range `src/llm/tools.ts:31-240` at `break` 91; the thin SDK adapter wrappers are intentionally outside that area. `src/llm/completion.ts` remains transport glue outside mutation testing.
- **Direct, aggregate, and changed routing.** `pnpm run analyze:mutation:mentor` selects the legacy mentor area, while `pnpm run analyze:mutation:graphTools` selects the graph-tools area through `NESSO_MUTATION_AREA` and the shared `scripts/stryker/mentor.mjs` config; its report lands in `reports/mutation/graph-tools/`. `pnpm run analyze:mutation` runs both in addition to the schema/store/workspace areas. `pnpm run analyze:mutation:changed` uses `scripts/stryker/areas.mjs` to map changed source/tests to the matching area, so `tools.ts` routes to `graphTools` and context/due-queue paths route to `mentor`; a changed shared area/config file can intentionally select both. This routing is part of the gate, not a combined mentor score.
- **Not a per-PR gate.** The per-mutant reruns are too slow for every push, so this runs **opt-in locally** (the direct commands above, `pnpm run analyze:mutation` for all areas, or **`pnpm run analyze:mutation:changed`** for changed areas) and on a **non-blocking scheduled job** (`.github/workflows/mutation.yml`, weekly + `workflow_dispatch`, one step per area), never as a required check. **Preflight:** run `analyze:mutation:changed` before push when the diff touches mutated pure logic (same conditional pattern as the Rust block in the preflight skill). Incremental mode (`stryker run --incremental` + a cached incremental file) is left for the scheduled job to opt into once full runs get expensive. Reports land in `reports/mutation/<area>/` (gitignored), including the graphTools report at `reports/mutation/graph-tools/`. The config file is a **positional** arg to `stryker run` — `-c` is Stryker's alias for `--concurrency`, not config.
...
- **Equivalent mutants.** Some mutations produce semantically identical code (a `typeof x !== 'number'` guard subsumed by a following `!Number.isFinite(x)`; an id-collision `Set` whose contents never collide; a referential-identity micro-optimisation in a no-op guard; a `.trim()` removed from already-trimmed input; a `<=`→`<` boundary at an unreachable exact value). No test can kill them, so 100% is unreachable by design. Note: Stryker's `MethodExpression` mutator drops the **last** call in a chain, so `x.replace(/…/).trim()` mutates to `x.replace(/…/)` — only whitespace-laden inputs catch it. **Schema note — optional fields in `JSON.stringify`:** a conditional spread like `...(doc.id !== undefined && { id: doc.id })` before `JSON.stringify` is an equivalent-mutant factory: removing the guard (`...(true && { id: doc.id })`) produces identical JSON because `JSON.stringify` omits `undefined` values on its own. The serializer should place optional properties directly in the object literal (`id: doc.id`) and rely on the built-in omission — eliminating the equivalent site without changing behaviour. Document known equivalents next to the threshold rather than chasing them or contorting source to kill them.
- **Baselines (per area).** Each `break` sits a couple points under the measured score. Raise it as the score climbs; when a change intentionally lowers it (deleting tested code), re-baseline the area's `break` in the same change.
  - `schema` — structural serialize/deserialize in `@nesso-how/schema`. The `meta` envelope field must be a record (plain object); non-record values (`null`, `false`, `0`, strings, arrays) are omitted from the deserialized document. Optional envelope fields (`vocabulary`, `id`, `updatedAt`, `meta`) are omitted from serialized JSON by `JSON.stringify`'s built-in `undefined` handling and conditionally included in the deserialized in-memory document so callers can distinguish absent keys. **`break` 89** at 100% measured; re-baseline only when the document shape changes (envelope additions/removals, concept/relation field changes).
  - `mentor` — legacy prompt/context and due-queue mutation area, `break` 84. Its known residuals are mostly equivalent truncation regex/boundary mutants.
  - `graphTools` — pure graph-query range in `src/llm/tools.ts`, `break` 91. The SDK adapter wrappers are not part of this mutation area.
