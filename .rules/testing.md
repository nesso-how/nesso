...
- **CI:** dedicated `e2e` (Playwright) and `e2e-native` (tauri-driver) jobs in `.github/workflows/ci.yml` run in parallel with `js`/`rust`. `e2e` uploads the HTML report on failure; `e2e-native` builds the pre-baked Docker image (`e2e-native/Dockerfile`) with a GHA layer cache and runs the lane inside it — no per-run apt, and byte-for-byte the same environment as a local `run-local.sh`. That image is what replaced the earlier slow, mirror-dependent apt install (the cached-apt action restored an incomplete runtime closure → `UND_ERR_SOCKET` session failures). The native job is **not yet a required check** — promote it in the repo ruleset once it is reliably green across a few runs.
...
- **Per-area configs, per-area floors.** A shared base (`stryker.base.mjs`) plus one config per area (`stryker.<area>.mjs`, built via `area()`), each with its own `mutate` and `break`. Thresholds are **per area, not aggregated**: a tiny well-tested package would otherwise mask a large less-tested one (the same reason `vitest.config.ts` keeps per-directory coverage floors). `mutate` is pure logic only — UI/React and the Rust layer stay out of scope. Each area is scoped to its _tested_ source and excludes glue: the store config takes the two tested slices (not `settings`/`ui`/`desktop-sync`/`index.ts`); the workspace config excludes the Tauri boundary (`watch.ts`, `scope.ts`) and the barrel; the mentor config takes `context.ts` + `fsrsDueQueue.ts` (not the network/WebGPU transports).
- **Not a per-PR gate.** The per-mutant reruns are too slow for every push, so this runs **opt-in locally** (`pnpm run analyze:mutation` for all areas, or `…:<area>` for one — always a full deterministic run) and on a **non-blocking scheduled job** (`.github/workflows/mutation.yml`, weekly + `workflow_dispatch`, one step per area), never as a required check. Incremental mode (`stryker run --incremental` + a cached incremental file) is left for the scheduled job to opt into once full runs get expensive. Reports land in `reports/mutation/<area>/` (gitignored). The config file is a **positional** arg to `stryker run` — `-c` is Stryker's alias for `--concurrency`, not config.
...
- **Baselines (per area).** Each `break` sits a couple points under the measured score. Raise it as the score climbs; when a change intentionally lowers it (deleting tested code), re-baseline the area's `break` in the same change.
  ...
  - `mentor` (`context.ts` + `fsrsDueQueue.ts`) — **86.50%** (141/163), residual mostly equivalent truncation regex/boundary; `break` 84.
