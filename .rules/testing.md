...
**WebDriver cannot drive native OS windows** — the folder/save **dialogs** (`pickWorkspaceFolder`) and the **native menu bar** are not DOM, so multi-project switching, native import/export, and menu commands stay covered by Vitest (`graph-management.desktop.test.ts`, `useDesktopMenu`), not this lane. The folder-picker dialog is Rust-owned and callback-based (`pick_folder` → `await_folder_picker`), keeping the Tauri runtime unblocked; the approval helper (`try_approve_folder`) is covered at the helper level by five focused Rust regression tests (`picker_approval_tests`) with injected callbacks: runtime progress while pending, successful grant-before-persist ordering, cancellation with no side effects, invalid selection with no side effects, and scope-grant failure without persistence. The save dialog (`save_file_dialog`) uses `blocking_save_file` and remains out of scope for the non-blocking rework. The external-conflict **banner** branch (unsaved local edits + an external write) needs deterministic timing control and is a follow-up; the no-local-edits reload branch is covered.
