...
- `@/…` → `src/…`.
- `@nesso-how/types` and `@nesso-how/vocab-learning` are aliased to their **source** (`packages/*/src/index.ts`) so tests run against source without a build.
...
- **Baselines (per area).** Each `break` sits a couple points under the measured score. Raise it as the score climbs; when a change intentionally lowers it (deleting tested code), re-baseline the area's `break` in the same change.
  - `formats` — **91.23%** (104/114), all 10 survivors equivalent (4 stable-key serialize conditionals whose `true`-mutant emits `key: undefined`, which `JSON.stringify` drops; the shared `asRecord` guard re-validated by every caller; the `typeof x !== 'number'` / `!Number.isFinite(x)` subsumption pair) → ~91% ceiling; `break` 89. Dropped from the pre-vocabulary 95.74% when `deserializeGraph` lost its killable node-normalization branches and the stable-key serializer added the JSON-drop conditionals.
  - `types` (`packages/types` runtime: `nodeToCard`, display merge) — **96.97%** (32/33), near ceiling; `break` 95. (`defaultConceptReviewFields` left the `types` runtime for `@nesso-how/vocab-learning`, which has no mutation area, so it no longer counts here.)
