...
Skills live at `.opencode/skills/<name>/SKILL.md`; subagents at `.opencode/agents/<name>.md`. Each carries valid frontmatter. A skill and a subagent may cover related ground — that is composition, not duplication. Primary agents (e.g. `nesso-brainstorm`, `nesso-fix`, `nesso-harness`, `nesso-work`) are also defined in `.opencode/agents/`.
...
An ordinary edit to the body of an existing rule does not require unrelated `AGENTS.md` churn when the Area-rules set and Touch → update paths are unchanged. It does require an agreement/reference check: links resolve, named source paths and symbols still exist, and the Area-rules/Touch-table bijection remains intact. If a rule is added, removed, renamed, or its trigger set changes, update `AGENTS.md` in the same change and audit the complete harness surface.
