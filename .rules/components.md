...
| `SidebarProjects`         | Desktop-only collapsible Projects section: `knownProjects` rows (active oxblood dot, switch on click, reveal-in-Finder / remove on hover), `+ New` (create) header, `Open project…` row.                   |
| `StatusBar`               | Full-width bottom strip (`STATUS_BAR_HEIGHT_PX`): Socrates entry + concept/relation counts or live selection (left); undo/redo · zoom · fit (right).                                                       |
...
| `GraphContextMenu`        | Right-click menu wired via `NessoGraph` `reactFlowProps` (node/edge/pane). Node: copy/cut/duplicate/delete. Edge: change type / flip direction / delete. Canvas: paste / add here / center-fit.            |
