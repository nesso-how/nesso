...
- **Desktop auto-updates:** the macOS job (`publish`) publishes the build as a **full release** (`prerelease: false`) — not a pre-release — so the updater endpoint `releases/latest/download/latest.json` resolves to it. It ships a signed, notarized universal macOS `.dmg` plus an **unsigned** x64 NSIS Windows installer (Authenticode signing is deliberately deferred; the release body warns the `.exe` may trigger Microsoft Defender SmartScreen). Both platforms sign updater artifacts with the same minisign keys — repo secrets `TAURI_SIGNING_PRIVATE_KEY` and `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`; the matching public key lives in `src-tauri/tauri.conf.json` (`plugins.updater.pubkey`) — and the `publish-windows` job (which runs after `publish`) merges a `windows-x86_64` entry into the same `latest.json`. Because Windows follows macOS instead of racing it, a failed Windows job can be rerun independently without touching the macOS release, and serialization prevents concurrent `latest.json` writes. Updates only reach users already on a build that bundles the updater plugin.
